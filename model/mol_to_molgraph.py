    ...
    if (len(atoms) == 1) or (radius == 0):
        ...
        for _ in range(radius):
            ...
            for i, j_bond in i_jbond_dict.items():
                ...
                fingerprint = (vertices[i], tuple(sorted(neighbors)))  # Aggregation of adjacent atom information (atom type, bond valence)
...
def bond_sterero_get(bond):
    if bond.GetStereo() in allowable_features['possible_bond_stereo']:
        ...
        # All other three-dimensional information is classified into one category, and the index is classified as 4.
        return [4]
...
if __name__ == "__main__":
    # Test whether bidirectional edges are generated
    atom = AtomF(atom_data_field='hv')
    bond = BondF(bond_data_field='he', self_loop=True)
