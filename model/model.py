...
class gasa_classifier(torch.nn.Module):
    ...
    def forward(self, g, motif_vocab, maccs, feature_3d, bg_vir_node, get_node_weight=True):
        ...
        g_feats, node_weights = self.readout(g, h, bg_vir_node, get_node_weight)
        ...
        hg = torch.sigmoid(self.predict(mixed_enbedding))

        return hg, node_weights
        ...
        with g.local_scope():
            ...
            for idx, g_one in enumerate(g_list):
                ...
                if len(bg_vir_atom[idx]) == 0:
                    ...
                    g.vir_atom.append(torch.zeros(1, fea, dtype=torch.float).to(device))
                else:
            ...
            offsets_vir_atom = torch.tensor([len(vir_atom) for vir_atom in bg_vir_atom], dtype=torch.int).to(device)
            offsets_atom = torch.tensor([vir_atom - len(bg_vir_atom[i]) for i, vir_atom in
                                         enumerate(g.batch_num_nodes().tolist())], dtype=torch.int).to(device)
            offsets_vir_atom = torch.tensor([vir_atom + 1 if vir_atom < 1 else vir_atom
                                             for vir_atom in offsets_vir_atom.tolist()], dtype=torch.int).to(device)
...
class WeightedSumAndMax(nn.Module):
    ...
    def forward(self, bg,  feats, bg_vir_atom, get_node_weight=True):
...
class Motif_trans(nn.Module):
    def __init__(self, num_embeddings=800, embedding_dim=20, padding_idx=400, d_model=20, nhead=4,
        ...
        self.out = nn.Linear(embedding_dim * 6, 10)
